...
* @Last Modified time: 2018-12-01 20:15:55
*/
...
class Main {
  constructor () {
...

    {
      ...
      // 测试快速排序
      function quickSortTest (quickSortVersion) {
        const n = 20;
        // 生成一个随机数组
        const arr = sortTestHeplper.generateRandomArrayByLengthAndReapetArea(n, 0, 100000);
        // 打印排序前的数组
      ...
      // that.alterLine("Quick Sort 3 Ways Area");
      // quickSortTest((arr) => sortAlgorithms.quickSort3Ways(arr));
    }

    {
        ...
        const n = 2000000;
        // 完全随机
        const array = sortTestHeplper.generateRandomArrayByLengthAndReapetArea(n, 0, 100000000);
        // 近乎有序的数组
        const swapTimes = 0;
        ...
        function testArrayPerformance (arrayVersion) {
          ...
          that.alterLine("Merge Sort Bottom Up Area");
          const mergeSortBottomUpInfo = testSortPerformance(sortAlgorithms.mergeSortBottomUp, arrayVersion);
          that.show(mergeSortBottomUpInfo);
          that.log(mergeSortBottomUpInfo);

          that.alterLine("Merge Sort Bottom Up 2 Area");
          const mergeSortBottomUp2Info = testSortPerformance(sortAlgorithms.mergeSortBottomUp2, arrayVersion);
          that.show(mergeSortBottomUp2Info);

          // that.alterLine("Quick Sort 1 Area"); // 在有序的情况下、在有重复元素的情况会 O(n^2)
          // const quickSortInfo = testSortPerformance(sortAlgorithms.quickSort, arrayVersion);
          // that.show(quickSortInfo);
          // that.log(quickSortInfo);

          // that.alterLine("Quick Sort 2 Area"); //  在有序的情况下、在有重复元素的情况会 O(n^2)
          // const quickSort2Info = testSortPerformance(sortAlgorithms.quickSort2, arrayVersion);
          // that.show(quickSort2Info);
          // that.log(quickSort2Info);

          // that.alterLine("Quick Sort 3 Area"); // 在有重复元素的情况下 会 O(n^2)
          // const quickSort3Info = testSortPerformance(sortAlgorithms.quickSort3, arrayVersion);
          // that.show(quickSort3Info);
          // that.log(quickSort3Info);

          // that.alterLine("Quick Sort 4 Area"); // 在有重复元素的情况下 会 O(n^2)
          // const quickSort4Info = testSortPerformance(sortAlgorithms.quickSort4, arrayVersion);
          // that.show(quickSort4Info);
          // that.log(quickSort4Info);

          that.alterLine("Quick Sort 2 Ways Area"); 
          const quickSort2WaysInfo = testSortPerformance(sortAlgorithms.quickSort2Ways, arrayVersion);
          that.show(quickSort2WaysInfo);
          that.log(quickSort2WaysInfo);

          that.alterLine("Quick Sort 2 Ways 2 Area"); 
          const quickSort2Ways2Info = testSortPerformance(sortAlgorithms.quickSort2Ways2, arrayVersion);
          that.show(quickSort2Ways2Info);
          that.log(quickSort2Ways2Info);

          that.alterLine("Quick Sort 3 Ways Area"); 
          const quickSort3WaysInfo = testSortPerformance(sortAlgorithms.quickSort3Ways, arrayVersion);
          that.show(quickSort3WaysInfo);
          that.log(quickSort3WaysInfo);
        }
...
class SortAlgorithms {
  ...
  mergeSortBottomUp (array) {
    ...
    for (let size = 1; size < len; size += size) {
      // 这里的i表示起始的指针，每一次跳动的步长是 当前位置 加上两倍 size
      // 第一次是 0 ，每次的步长是 2* size，假设size是 1
      // 第二次是 2 ，每次的步长是 2* size, 
      // 第三次是 4 ，每次的步长是 2* size,
      // 第四次是 8
      // 整个过程依次类推， 假设size为2
      // 第一次是 0，第二次4，第三次 8，
      // 它有一个自底向上的过程
      for (let i = 0; i + size < len; i += size * 2) {
        // 每次合并都是 从0开始
        // 对 左半部分 array[i...i+size-1] 与 右半部分 array[i+size...i+size+size-1]
        // 进行归并，
        // 将两个有序的部分 合并成 一个有序的部分，
        // 如果只有一部分的话，它本身已经有序了，那么就达到了运行完merge方法的要求了，
        // 所以要保证 i + size 不会越界，那么就要在循环中 从 i 改为 i + size，
        // 这样一来middle就不会越界，但是right有可能会越界，
        // 那么就需要取 right 和 len - 1 之间的最小值
        this.mergeBU(array, i, i + size - 1, Math.min(i + size + size - 1, len - 1));
      }
    }
  }
  
  // 归并排序 合并操作 -
  mergeBU (array, left, middle, right) {
    // 会对数组的空间进行压缩处理，如过不进行压缩处理 就像merge2 一样
  ...
  // 优化一 小部分的数组 使用插入排序
  // 优化二 防止 加法操作整型溢出
  // 优化三 对于一个数组的中间部分边界的两个元素进行比较，如果这两个元素有序，说明没有必要继续合并了
  mergeSortBottomUp2 (array) {
    const len = array.length;

    // 对于小数组，使用插入排序来优化
    for (let i = 0; i < len; i += 16) // 不要过界
      this.insertionSortByFragment(array, i, Math.min(i + 15, len - 1));

    for (let size = 16; size < len; size += size)
      for (let i = 0; i< len - size; i += size * 2)  // 优化 2 将 (i + size < len) 进行优化
        // 一样的，对于array[middle] <= array[middle+1]的情况，就说明不需要进行优化
        if (array[i + size - 1] > array[i + size]) //优化3 反之就需要进行优化  
          this.mergeBU2(array, i, i + size - 1, Math.min(i + size + size - 1, len - 1));
  }
  
  // 归并排序 合并操作 -
  mergeBU2 (array, left, middle, right) {
    // 会对数组的空间进行压缩处理，如过不进行压缩处理 就像merge2 一样
  ...
  quickSort (array) {
    const len = array.length;
    this.recursiveQuickSort(array, 0, len - 1);
  }
  ...
  recursiveQuickSort (array, left, right) {
    ...
    // 执行一个分界的方法，并且返回分左右部分的中间值
    const p = this.partition2(array, left, right);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition (array, left, right) {

    const v = array[left]; // 取数组中 左边的第一个值来作为 比较的标识
    let j = left; // 这是那个最后决定用来分割数组左右两部分的那个中间值的索引
    ...
    // 交换一下j 和 left  也就是用 v 去隔开 数组的左右两部分
    this.swap(array, left, j);

    return j; // 返回这个分隔数组左右部分的 v 的索引 j
  }

  // 快速排序
  // 优化： 如果 partition2中的指针i 和指针j 相同 就不进行交换
  // 效果： 快一丁点
  quickSort2 (array) {
    const len = array.length;
    this.recursiveQuickSort2(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort2 (array, left, right) {
    // 如果越界了，那就终止
    if (left >= right)
      return;
    ...
    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort2(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort2(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition2 (array, left, right) {

    const v = array[left]; // 取数组中 左边的第一个值来作为 比较的标识
    let j = left; // 这是那个最后决定用来分割数组左右两部分的那个中间值的索引

    for (let i = left + 1; i <= right; i++)
      if (array[i] < v && i !== ++ j) // 优化 如果i 和 j 相等 那么就不需要交换了
        this.swap(array, i, j);
  ...
  // 快速排序
  // 原因一：因为快速排序中取的标识总是左边部分的第一个元素，
  // 如果数组是有序的，那么问题就来了，左部分的第一个元素肯定是最小的，
  // 那么返回回去的中间元素索引肯定是非常的小，这样就会导致 左右两部分严重的不平衡，
  // nlogn级别复杂度的算法 一般都和树有关，如果一棵树严重不平衡，
  // 那么就有可能会退化成链表，时间复杂度就会是(n^2)。
  // 优化： 让取标识时的位置变成随机的，
  // 效果： 在有序的情况下时间复杂度也不会变成O(n^2)了
  quickSort3 (array) {
    const len = array.length;
    this.recursiveQuickSort3(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort3 (array, left, right) {
    // 如果越界了，那就终止
    if (left >= right)
      return;

    // 执行一个分界的方法，并且返回分左右部分的中间值
    const p = this.partition3(array, left, right);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort3(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort3(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition3 (array, left, right) {

    // 优化部分
    const randomValue = Date.now() & 0x7fffffff; // 通过日期来获取随机值
    // 改变标识 改变的范围在left+1 ~ right 任意范围之内
    this.swap(array,  randomValue % (right - left + 1) + left, left);
  ...
  // 快速排序
  // 优化： 当元素过少时 对数组片段 进行插入排序的 优化操作，
  // 效果： 性能得到了
  quickSort4 (array) {
    const len = array.length;
    this.recursiveQuickSort4(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort4 (array, left, right) {
    // 优化
    if (right - left <= 15 ) { // 当左右部分的元素的范围在15之内就使用插入排序进行优化
      this.insertionSortByFragment(array, left, right);
      return;
    }

    // 执行一个分界的方法，并且返回分左右部分的中间值
    const p = this.partition4(array, left, right);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort4(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort4(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition4 (array, left, right) {
  ...
  // 快速排序 双路排序
  // 原因： 当数组中出现大量重复元素就会是O(n^2)
  quickSort2Ways (array) {
    const len = array.length;
    this.recursiveQuickSort2Ways(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort2Ways(array, left, right) {
    // 优化
    if (right - left <= 15 ) { // 当左右部分的元素的范围在15之内就使用插入排序进行优化
      this.insertionSortByFragment(array, left, right);
      return;
    ...
    // 执行一个分界的方法，并且返回分左右部分的中间值
    const p = this.partition2Ways(array, left, right);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort2Ways(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort2Ways(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition2Ways(array, left, right) {
    ...
    const v = array[left]; // 取数组中 左边的第一个值来作为 比较的标识
    let i = left + 1; // i 指向比 v 大的元素的索引 
    let j = right;
    while (true) { // 优化思路 ： i < j  j > i

      // 从左端开始遍历，直到找到 比 v大的元素 就停止 
      // 这里的条件是 小于等于 right 并且 指针指向的数组元素值 小于 v
      while (i <= right && array[i] < v)
        i ++; // 指针向后移动， 遇到 比 v大的元素就停下来

      // 从右端开始遍历，直到找到 比 v小的元素 就停止
      // 这里的条件是 大于 left 并且 指针指向的数组元素值 大于 v
      while (j > left && array[j] > v) /// j > left 等价于 j >= left + 1
        j --; // 指针向前移动， 遇到 比 v小的元素就停下来
      ...
      // 到了这里 
      // 就说明  从左端遍历到右端，终于找到了 比v大的元素的索引了， 暂时存到 i里面
      // 就说明  从右端遍历到左端，终于找到了 比v小的元素的索引了， 暂时存到 j里面
      
      // 开始交换
      this.swap(array, i, j);

      // 交换之后 维护一下 指针 i 和 j 的值
      i ++;
      j --;
    }

    // 选择j 是因为 array[j] 是在小于等于标定点v的这一端
    this.swap(array, left , j);
  ...
  // 快速排序 双路排序
  // 优化： 对两端指针进行相互的牵制，这样就不会出现越界后还能继续跑
  quickSort2Ways2 (array) {
    const len = array.length;
    this.recursiveQuickSort2Ways2(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort2Ways2(array, left, right) {
    // 优化
    if (right - left <= 15 ) { // 当左右部分的元素的范围在15之内就使用插入排序进行优化
      this.insertionSortByFragment(array, left, right);
      return;
    ...
    // 执行一个分界的方法，并且返回分左右部分的中间值
    const p = this.partition2Ways2(array, left, right);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort2Ways2(array, left, p - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort2Ways2(array, p + 1, right); // 对右边部分进行快速排序
  }

  // 将一个数组分成两半，并且返回分隔两部份的那个中间元素的索引 -
  partition2Ways2(array, left, right) {
    ...
    while (true) { // 优化思路 ： i < j  j > i
      ...
      // 从左端开始遍历，直到找到 比 v大的元素 就停止 
      // 这里的条件是 小于等于 j 并且 指针指向的数组元素值 小于 v
      while (i <= j && array[i] < v)
        i ++; // 指针向后移动， 遇到 比 v大的元素就停下来

      // 从右端开始遍历，直到找到 比 v小的元素 就停止
      // 这里的条件是 大于 i 并且 指针指向的数组元素值 大于 v
      while (j > i && array[j] > v) 
        j --; // 指针向前移动， 遇到 比 v小的元素就停下来
  ...
  quickSort3Ways (array) {
    const len = array.length;
    this.recursiveQuickSort3Ways(array, 0, len - 1);
  }

  // 快速排序 递归辅助函数 -
  recursiveQuickSort3Ways(array, left, right) {
    // 优化
    if (right - left <= 15 ) { // 当左右部分的元素的范围在15之内就使用插入排序进行优化
      this.insertionSortByFragment(array, left, right);
      return;
    ...
    //   if (array[i] < v) {
    //     this.swap(array, i, lt + 1);
    //     i ++;
    //     lt ++;
    //   } else if (array[i] > v) {
    //     this.swap(array, i, gt - 1);
    //     gt --;
    //   } else {
    //     i ++;
    //   }
    ...
    for (let i = lt + 1; i < gt; i ++) {
        if (array[i] < v) { // 当前元素小于v
          this.swap(array, i, lt + 1);
          lt ++;
        } else  if (array[i] > v) { // 当前元素大于v
          this.swap(array, i, gt - 1);
          i --;
          gt --;
        }
    ...
    this.swap(array, left, lt);

    // 求出这个分界值之后，再继续对左右两部分进行快速排序
    this.recursiveQuickSort3Ways(array, left, lt - 1); // 对左边部分进行快速排序
    this.recursiveQuickSort3Ways(array, gt, right); // 对右边部分进行快速排序
  }
